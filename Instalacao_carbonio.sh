...
domain=$(hostname)
certs_dom=$(hostname)
# Para adicionar outros subdomínios no servidor, você deve escrever na varaiavel dom_list com o padrão -d $(hostname) -d carboniossl.dominio.com.br -d novoexemplo.com
# Sempre lembrando que para o certificado ser gerado, o subdomínio ou o domínio devem apontar para o ip do servidor.
dom_list="-d $(hostname)"
cert_path="/root/.acme.sh/$(hostname)"
zerossl="no"
letsencrypt="yes"

if [ "$domain" = "" ] ; then
        echo "Set the variables above"
        exit
fi
...
# 2024-10-03 - Ricardo - Segundo o Lucas Jose vamos adicionar o --force para gerar os ssls. Entao mesmo que um dominio nao aponte para o servidor, irá gerar o ssl.
if [ "$zerossl" = "yes" ] ; then
        ./acme.sh --register-account -m zerossl@$domain
        ./acme.sh --issue --standalone --force --keylength 2048 $dom_list
        if [ "$?" != "0" ] ; then
                echo ""
                echo "Error to generate certificate to domain $domain"
                exit 1
        fi
fi
...
if [ "$zerossl" = "yes" ] ; then
        ...
        cat USERTrustRSACertificationAuthority.crt fullchain.cer > zextras_ca.pem
fi

chown zextras: /tmp/$tmp_path.$domain -R
su - zextras -c"cd /tmp/$tmp_path.$domain ; /opt/zextras/bin/zmcertmgr verifycrt comm $certs_dom.key $certs_dom.cer zextras_ca.pem"
if [ "$?" != "0" ] ; then
        echo ""
        echo "Error testing certificate to domain $domain"
        exit 1
fi
...
su - zextras -c"cd /tmp/$tmp_path.$domain ; /opt/zextras/bin/zmcertmgr deploycrt comm $certs_dom.cer zextras_ca.pem"
